...
export default function ChampionshipDetail({ onNavigate }: ChampionshipDetailProps) {
  ...
  const { media: galleryMedia, isLoaded: mediaLoaded } = useAfterMovieMedia("championship")
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <button 
        ...
        className="fixed top-20 left-4 z-30 text-white/70 hover:text-red-500 transition-colors"
        aria-label="Go back"
      >
      ...
      {/* Hero Section */}
      <section className="relative min-h-[60vh] sm:min-h-[70vh] md:min-h-[80vh] flex flex-col justify-end pb-6 sm:pb-8 md:pb-10">
        {/* Background Image - Fighter in ring */}
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{
            backgroundImage: `url('https://f005.backblazeb2.com/file/b21of1firm/background/CHAMPhome.png')`,
        ...
        {event?.soldOut && (
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
            <div className="bg-red-600 px-6 sm:px-10 py-2 sm:py-3 shadow-lg rotate-45">
        ...
        {/* Content */}
        <div className="relative z-10 px-3 sm:px-4 md:px-8 mt-20 sm:mt-24">
          <span className="text-red-600 text-[10px] sm:text-xs tracking-[0.2em] sm:tracking-[0.3em] mb-1 sm:mb-2 block font-medium">SIGNATURE EVENT</span>
          <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-8xl font-thin tracking-wide sm:tracking-wider mb-1 text-white leading-none" style={{ fontFamily: '"Inter", sans-serif' }}>
            {event?.name || "THE 1 OF 1"}<br />{event?.subtitle || "CHAMPIONSHIP"}
          </h1>
          
          {/* Tagline */}
          <div className="border-l-2 border-red-600 pl-3 sm:pl-4 mb-4 sm:mb-6">
            <p className="text-white/80 text-xs sm:text-sm tracking-wider sm:tracking-widest leading-relaxed">
              4 SATURDAYS.<br />
          ...
          <div className="space-y-2 sm:space-y-3 max-w-xs">
            <div className="flex items-center gap-2 sm:gap-3">
              <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/60 flex-shrink-0" />
              <span className="text-white/80 text-xs sm:text-sm tracking-wider">{event?.date || "COMING SOON"}</span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              <MapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/60 flex-shrink-0" />
              <span className="text-white/80 text-xs sm:text-sm tracking-wider">{event?.location || "BARRANQUILLA"}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Countdown Section */}
      <section className="px-3 sm:px-4 py-4 sm:py-6 max-w-2xl mx-auto">
        <div className="bg-black/60 border border-white/10 rounded-lg p-3 sm:p-4 flex flex-col md:flex-row items-center gap-3 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full border border-white/20 flex items-center justify-center">
              <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-white/60" />
            </div>
            <div>
              <span className="text-[7px] sm:text-[8px] md:text-[10px] tracking-wider text-white/60 block">FALTA PARA</span>
              <span className="text-[9px] sm:text-[10px] md:text-xs tracking-wider text-white">THE 1 OF 1</span>
              <span className="text-[9px] sm:text-[10px] md:text-xs tracking-wider text-white block">CHAMPIONSHIP</span>
            </div>
          </div>
      ...
      <section className="px-3 sm:px-4 py-4 sm:py-6 max-w-lg mx-auto">
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 mb-4 sm:mb-6">
          ...
          <span className="text-white/60 text-[9px] sm:text-[10px] md:text-xs tracking-[0.1em] sm:tracking-[0.15em] md:tracking-[0.2em] whitespace-nowrap">{event?.stage || "ETAPA CREYENTES"}</span>
          <div className="h-px bg-white/20 flex-1" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {/* Full Pass */}
          <div className="border border-white/20 p-3 sm:p-4 text-center">
            <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 mx-auto mb-2 sm:mb-3 border border-red-500/50 rounded flex items-center justify-center">
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <h3 className="text-white font-medium tracking-wider text-[10px] sm:text-xs md:text-sm mb-1">FULL PASS</h3>
            <p className="text-white/50 text-[7px] sm:text-[8px] md:text-[10px] tracking-wider mb-2 sm:mb-3 leading-tight">ACCESO A LAS 4 FECHAS<br />Y TODOS LOS COMBATES</p>
          ...
          {/* Fight Pass */}
          <div className="border border-red-500/50 p-3 sm:p-4 text-center">
            <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 mx-auto mb-2 sm:mb-3 border border-red-500/50 rounded flex items-center justify-center">
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-red-500" viewBox="0 0 24 24" fill="currentColor">
                <path d="M7 11V7a5 5 0 0110 0v4m-9 0h8a2 2 0 012 2v6a2 2 0 01-2 2H8a2 2 0 01-2-2v-6a2 2 0 012-2z" />
              </svg>
            </div>
            <h3 className="text-white font-medium tracking-wider text-[10px] sm:text-xs md:text-sm mb-1">FIGHT PASS</h3>
            <p className="text-white/50 text-[7px] sm:text-[8px] md:text-[10px] tracking-wider mb-2 sm:mb-3 leading-tight">ACCESO A 1 FECHA<br />Y TODOS LOS COMBATES</p>
            <div className="mb-2 sm:mb-3 md:mb-4">
        ...
        {/* Disclaimer */}
        <div className="flex items-center justify-center gap-1 sm:gap-2 mt-3 sm:mt-4 text-white/40 text-[7px] sm:text-[8px] md:text-[10px] tracking-wider px-2">
      ...
      <section className="px-3 sm:px-4 py-4 sm:py-6">
        <div
          className="relative py-5 sm:py-6 md:py-12 px-3 sm:px-4 md:px-8 border border-white/20 overflow-hidden rounded-lg"
          style={{
            backgroundImage: `url('https://f005.backblazeb2.com/file/b21of1firm/background/CHAMPtarj.jpg')`,
            backgroundSize: "cover",
            backgroundPosition: "center",
      ...
      <section className="px-3 sm:px-4 py-4 sm:py-6">
        <div
          ...
          style={{
            backgroundImage: `url('https://f005.backblazeb2.com/file/b21of1firm/background/CHAMPtarj2.jpg')`,
            backgroundSize: "cover",
            backgroundPosition: "center top",
            backgroundRepeat: "no-repeat",
          }}
        >
          <div className="absolute inset-0 sm:bg-gradient-to-r sm:from-black/60 sm:via-black/40 sm:to-transparent bg-gradient-to-r from-black/70 via-black/50 to-black/30" />
          <div className="relative z-10 flex items-center justify-start">
            <div>
              <span className="text-red-500 text-[7px] sm:text-[8px] md:text-[10px] tracking-widest">REVIVE LA EXPERIENCIA</span>
              <h3 className="text-base sm:text-lg md:text-2xl lg:text-3xl font-bold tracking-wider mt-1 uppercase">THE 1 OF 1 CHAMPIONSHIP</h3>
              <button 
                onClick={() => setIsGalleryOpen(!isGalleryOpen)}
                className="flex items-center gap-2 mt-2 sm:mt-3 text-white/80 hover:text-red-500 transition-colors"
              >
                <Play className="w-3 h-3 sm:w-4 sm:h-4" />
                <span className="text-[9px] sm:text-[10px] md:text-xs tracking-widest">VER AFTERMOVIE</span>
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={closeLightbox}>
          ...
          <button onClick={(e) => { e.stopPropagation(); goToPrevious(); }} className="absolute left-2 md:left-6 z-50 text-white/70 hover:text-red-500 transition-colors p-2">
          ...
          <button onClick={(e) => { e.stopPropagation(); goToNext(); }} className="absolute right-2 md:right-6 z-50 text-white/70 hover:text-red-500 transition-colors p-2">
      ...
      {/* Footer */}
      <footer className="px-3 sm:px-4 py-4 sm:py-6 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 border-t border-white/10">
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-center sm:text-left">
          <div className="text-white">
            <div className="text-sm sm:text-lg font-bold tracking-tight">
              <span className="text-red-500">1</span>
              <span className="text-[9px] sm:text-[10px] md:text-xs align-top mx-0.5">OF</span>
              <span className="text-red-500">1</span>
              <span className="text-[9px] sm:text-[10px] md:text-xs ml-1">FIRM</span>
            </div>
          </div>
          <div>
            <p className="text-white/60 text-[9px] sm:text-[10px] md:text-xs">THIS IS NOT FOR EVERYONE.</p>
            <p className="text-red-500 text-[9px] sm:text-[10px] md:text-xs tracking-wider">#1UNIQUEEXPERIENCE</p>
          </div>
        </div>
